...
while end:
    ...
    print("here")
    # if game ends displays game over text
    game_over = True
    while game_over:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
        ...
        pygame.draw.rect(win, (180, 255, 100), (width/2 - 100, height/2 - 25, 200, 50), 0)

        restart_button = font.render("RESTART?", True, (128, 128, 128))
        win.blit(restart_button, ((width - restart_button.get_rect().width)/2, (height - restart_button.get_rect().height)/2))
        mouse = pygame.mouse.get_pos()
        mouse_click = pygame.mouse.get_pressed()
        if mouse_click[0] == 1 and width/2 <= mouse[0] <= width/2 + 200 and height/2 <= mouse[1] <= height/2 + 50:
            end = True
            game_over = False
